...
It is use case no. 3 that we consider for its potential musicality and the applicability of musical-inspired pattern design. These patterns must be suitable for low resolution vibrotactile arrays -- in our case a 2x5 circular array of vibe motors, or tactors, that encircle the wrist (Figure ~\ref{fig:braceletRendering}). Mapping of types of things that are commonly expressed using standard musical notation (music notation), entities such as notes with durations, parts, and instruments need to translated, or mapped, from the music domain to that of vibrotactile devices. This mapping has proven to be useful and appears to lead to interesting possibilities for pattern design for these devices.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Designing patterns using music notation}

\subsection{Introduction}
This research employs the text-based music engraving and compositional tool Lilypond \cite{nienhuys2003lilypond}. The application Frescobaldi [http://frescobaldi.org] was used as a front-end to Lilypond. Writing music using Lilypond is a similar process to writing papers using the document preparation system LaTex. Like LaTex, its textual input can be easily version-controlled. The output of Lilypond follows music notation conventions and is of high quality. Once learned to a rudimentary level Lilypond is a useful tool for experimenting with music notation. Lilypond can easily output MIDI code. This MIDI is used to drive the actuator array in the vibrotactile bracelet using the standard Arduino MIDI library. The complete workflow is to first compose music in Frescobaldi/Lilypond, then output a MIDI file from the Lilypond source, import this MIDI file into a track in Ableton Live, send a MIDI stream out from Ableton into a dedicated USB MIDI cable and then read this MIDI data in Arduino, using MIDI-read() function. The workflow proved to be quite workable as an experimental technique.
...
\textit{You're getting warmer / you're getting colder}. These are spatial and metaphorical in nature and refer to navigating through some kind of virtual space. Panëels discusses vibrotactile design for metaphorical patterns such as \textit{turn back} on devices that have actuators with spatial patterns, but does not do it in a musical context \cite{paneels2013tactiped}. Ideally these gameplay clues are either self-explanatory or easy for the player to learn. Such clues should probably incorporate some notion of metaphorical movement through a space. For \textit{Go this way} the musical should point in the intended direction. A vibrotactile device simply having a series of tactors activating in the intended direction might be a good first step, much like a flashing series of traffic lights scrolling to the right might suggest to proceed to the right (Figure ~\ref{fig:tactorArray}).\\

Music as it is commonly notated does not lend itself to these sorts of concepts or metaphors. It is unclear at first what music that could be interpreted as \textit{go this way} might look like. Like in any design or artistic domain there are many solutions to a given problem. There are probably many direct and indirect artistic means for designing such a pattern. Although music that has spatial aspects is common, such as the surround sound of Thomas Tallis' \textit{Spem in Alium} [1570], or the call and response of antiphonal works, standard music notation tends not to focus on the specification of spatial aspects in music performance even though aural perception is intensely spatial. Instrument sections in ensembles are usually placed together in clusters but this is a performance convention and is usually outside of the notation itself.  
...
Music with one dimension is that which instruments follow some kind of spatial linear progression, such as pan or a sweep. Figure ~\ref{fig:arrowsMoving00}. shows one vibe motor activating in a simple rhythm with a simple, rhythmic pitch change. By the conventions of music notation it is clear that there is a regular repetitive rhythm represented. Of course, to know this requires at least a rudimentary knowledge of music notation.\\ 
